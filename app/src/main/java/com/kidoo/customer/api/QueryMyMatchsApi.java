...
public class QueryMyMatchsApi {

    public static Observable<KidooApiResult<MyMatchsResult>> queryMatchS(String customerId2Query, String pageNo, String pageSize) {
        ...
        Observable<KidooApiResult<MyMatchsResult>> observable = requset.call(kidooApiService.queryMyMatchList(requset.getParams().urlParamsMap));

        return observable;
    }

    public static Observable<KidooApiResult<MyMatchsResult>> queryMatchS(String customerId2Query) {
        CustomerRequset requset = KidooApiManager.custom().build()
                .params("customerId2Query", customerId2Query)
                .params("customerId", String.valueOf(AccountHelper.getUserId()))
                .params("token", TokenManager.getInstance().getToken());
