...
import com.kidoo.customer.api.QueryMatchManagerApi;
...
public class UserMatchsManagerInteractor implements UserMatchsManagerContract.Interactor {

    @Inject
    public UserMatchsManagerInteractor() {

    }

    @Override
    public Disposable queryMoreMatchsManagerAction(long customerId2Query, int pageSize, int pageNo, final GetMatchsCallback callback) {
        Observable<KidooApiResult<MyMatchsResult>> observable = QueryMatchManagerApi.queryMatchManager(String.valueOf(customerId2Query)
    ...
    @Override
    public Disposable queryMacthsManager(long customerId2Query, final GetMatchsCallback callback) {
        Observable<KidooApiResult<MyMatchsResult>> observable = QueryMatchManagerApi.queryMatchManager(String.valueOf(customerId2Query));
        Disposable disposable = observable.subscribe(new Consumer<KidooApiResult<MyMatchsResult>>() {
            @Override
