...
public class SharedPreferenceHelper  {
    ...
    public static <T> boolean save(Context context, T t) {
        SharedPreferences.Editor editor = buildNewEditor(context, t);
    ...
    private static <T> SharedPreferences.Editor buildNewEditor(Context context, T t) {
        ...
        for (String key : keys) {
            ...
            try {
                if (value == null) {
                    ...
                    LogUtils.d(TAG, String.format("Con't support save this type:%s, value:%s, key:%s",
                            type, value, key));
                }
            } catch (IllegalArgumentException e) {
                LogUtils.d(TAG, "Save error:" + e.getMessage());
            }
        }
    ...
    private static boolean saveEditor(SharedPreferences.Editor editor, boolean isAsync) {
        if (isAsync) {
            SharedPreferencesCompat.EditorCompat.getInstance().apply(editor);
            return true;
        } else
    ...
    private static <T> void buildValuesToMap(Class<?> clx, T t, String preFix,
        ...
        for (Field field : fields) {
            ...
            try {
                ...
                LogUtils.d(TAG, "buildValuesToMap error:" + e.getMessage());
                continue;
            }
        ...
        for (Field field : fields) {
            ...
            if (value != null) {
                try {
                    ...
                    e.printStackTrace();
                    LogUtils.d(TAG, String.format("Set field error, Key:%s, type:%s, value:%s",
                            key, fieldType, value));
                }
            } else {
                LogUtils.d(TAG, String.format("Get field value error, Key:%s, type:%s",
                        key, fieldType));
            }
        }
