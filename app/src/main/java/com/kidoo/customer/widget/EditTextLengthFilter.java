...
public class EditTextLengthFilter implements InputFilter {
    ...
    public EditTextLengthFilter(Context context, int length, TextView showNum, String toast) {
        init(context, length, toast, showNum);
    }
     ...
     * @param showNum 显示文字变化的TextView
     */
    private void init(Context context, int length, String toast, TextView showNum) {
        mContext = context;
        isMupli = length % 2 == 0;
        maxLength = isMupli ? length / 2 : length;
        mToast = toast != null ? toast : context.getResources().getString(R.string.team_modify_msg_limit);
        tvShowNum = showNum;
    }
